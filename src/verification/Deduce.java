...
//					System.out.println(pd.getStr());
			...
			//stage 1~stageSum
				...
				CPU.clear();
				int lastStage = 0;
				for (int i = 0; i < CPU.stageRange.length; i++){
					if (CPU.stageRange[i] == stage){
						lastStage = CPU.stageRange[i - 1];
						break;
					}
				...
				for (int num = 0; num < Proof.size(lastStage); num++){
					Procedure pd = Proof.get(lastStage, num);
					if (pd.getFormula().isRegContentFormula()){
